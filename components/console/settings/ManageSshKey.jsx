...
export default function ManageSshKey() {
  ...
  const deleteSshKey = (id) => {
    setSshKeys((prevKeys) => prevKeys.filter((key) => key.id !== id));
  };

  const copySshKey = async (sshKey) => {
    try {
      await navigator.clipboard.writeText(sshKey);
      toast(`SSH Key has been copied to clipboard.`);
    } catch (error) {
      console.error("Failed to copy SSH Key:", error);
      toast("Failed to copy SSH Key.");
    }
  };
  ...
  return (
    <div className="text-black p-14 mx-10">
      <p className="text-[16px] font-medium text-[#396A82]">
        Settings &gt;{" "}
        <span className="text-[16px] font-semibold text-[#41515C]">
          Manage SSH Keys
        </span>
      </p>
      <hr />
      ...
      <div className="relative overflow-x-auto shadow-md p-4 rounded-[8px]">
        <div className="flex flex-col space-y-4">
          ...
          {sshKeys.map((key) => (
            <div
              key={key.id}
              className="flex justify-between items-center bg-white border-b text-[#8D8D8D] font-semibold px-6 py-4"
            >
              ...
              <div className="relative">
                ...
                >
                  ...
                  >
                    <path
                      d="M5.25 6L0.703367 0.75L9.79663 0.75L5.25 6Z"
                      fill="black"
                    />
                  </svg>
                </button>
                {openPopupId === key.id && (
                  <div
                    ...
                    style={popupStyles}
                  >
                    <div className="flex items-center hover:bg-gray-100 px-2" onClick={() => copySshKey(key.sshKey)}>
                      <Image
                        src={require("@/public/settings/copy.png")}
                        alt="copyicon"
                      />
                    ...
                    >
                      <Image
                        src={require("@/public/settings/delete.png")}
                        alt="deleteicon"
                      />
